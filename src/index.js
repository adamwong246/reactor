...
// Import all stories statically
import ButtonStory from './stories/Button.stories.js';
import HeaderStory from './stories/Header.stories.js';
...
function App() {
    ...
    useEffect(() => {
        ...
        // Map story paths to their imported components
        ...
        config.stories.forEach(storyPath => {
            ...
            for (let i = 0; i < parts.length; i++) {
                const part = parts[i];
                if (i === parts.length - 1) {
                    // It's the story file - store the full path and component
                    current[part] = { 
                        path: storyPath,
                        component: storyComponents[storyPath]
                    };
                } else {
                    // It's a directory
                    if (!current[part]) {
    ...
    const currentStoryData = selectedStory ? findStoryInTree(storiesTree, selectedStory) : null;
    const CurrentStoryComponent = currentStoryData ? currentStoryData.component : null;
